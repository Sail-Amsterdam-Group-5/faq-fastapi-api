...
def mock_query_entities(query_filter):
    ...
    if query_filter == "PartitionKey eq 'General'":
        ...
        return [
            mock_faqs[1]
        ]  # Return only the second FAQ if filtering by 'Installation'
    elif (
        query_filter is not None
        and query_filter != "PartitionKey eq 'General'"
        and query_filter != "PartitionKey eq 'Installation'"
        and query_filter != ""
    ):
        return []
    else:
...
def test_get_faqs_by_non_existing_category():
    response = client.get(
        "/faqs", headers={"accept": "application/json", "category": "adawddwwefwef"}
    )
